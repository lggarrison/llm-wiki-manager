...
Wiki subcommands accept `--wiki-dir` and `--repo-root`. `build` and `check` use the schema's full entity-family type set when generating or validating `index.md`. See [Wiki Management Scripts](../concepts/wiki-scripts.md) for flags and npm script aliases.
