...
Upgrade refreshes meta paths in `WIKI_META_UPGRADE_PATHS`; existing `.entity-scopes` files are user-maintained and skipped during refresh, while missing ones are backfilled from focus directories. Init-only paths (`index.md`, `log.md`) are written on first init but not overwritten on re-init.
