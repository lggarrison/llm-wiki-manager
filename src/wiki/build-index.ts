import { readFileSync, writeFileSync, existsSync } from 'fs';
import { join, relative } from 'path';
import { parseFrontmatter } from './frontmatter.js';
import { walkMd } from './walk.js';
import { BUILD_INDEX_SKIP } from './constants.js';
import { formatIndexMarkdown } from './format-index.js';
import { resolveWikiContext } from './context.js';
import type { Frontmatter } from './frontmatter.js';
import type { WikiContext } from './context.js';

type PageEntry = { file: string; rel: string; fm: Frontmatter };
const FIRST_GENERATED_HEADING = '## Overview & Hub Pages';

// Tolerate CRLF: consumer repos rarely force eol=lf, so git may check
// index.md out with CRLF while the generator produces LF.
function normalizeEol(content: string): string {
  return content.replace(/\r\n/g, '\n');
}

function row(cells: string[]): string {
  return `| ${cells.join(' | ')} |`;
}

function generatedSectionStart(content: string): number | null {
  const normalized = normalizeEol(content);
  const headingIndex = normalized.indexOf(`\n${FIRST_GENERATED_HEADING}\n`);
  if (headingIndex >= 0) {
    const separatorIndex = normalized.slice(0, headingIndex).lastIndexOf('\n---\n');
    return separatorIndex >= 0 ? separatorIndex + 1 : headingIndex + 1;
  }

  const match = /(?:^|\n)---\n/.exec(normalized);
  if (!match) return null;
  return match[0].startsWith('\n') ? match.index + 1 : match.index;
}

function preserveIndexPreamble(generatedOutput: string, existing: string): string {
  const existingStart = generatedSectionStart(existing);
  const generatedStart = generatedSectionStart(generatedOutput);
  if (existingStart === null || generatedStart === null) return generatedOutput;

  const preamble = normalizeEol(existing).slice(0, existingStart).trimEnd();
  if (!preamble) return generatedOutput;

  return `${preamble}\n\n${normalizeEol(generatedOutput).slice(generatedStart)}`;
}

function buildIndexOutput(wikiDir: string): { output: string; pages: PageEntry[] } {
  const pages = walkMd(wikiDir)
    .filter((f) => !BUILD_INDEX_SKIP.some((s) => f.endsWith(s)))
    .map((f) => {
      const content = readFileSync(f, 'utf8');
      const fm = parseFrontmatter(content) ?? {};
      return { file: f, rel: relative(wikiDir, f).replace(/\\/g, '/'), fm };
    })
    .filter((p) => p.fm.type);

  const byType: Record<string, PageEntry[]> = {
    hub: [],
    entity: [],
    concept: [],
    source: [],
  };

  for (const page of pages) {
    if (page.fm.type === 'hub') byType.hub.push(page);
    else if (page.fm.type === 'overview' && page.rel.startsWith('entities/'))
      byType.entity.push(page);
    else if (page.fm.type === 'concept') byType.concept.push(page);
    else if (page.fm.type === 'source') byType.source.push(page);
  }

  for (const bucket of Object.values(byType)) {
    bucket.sort((a, b) =>
      ((a.fm.title as string) ?? '').localeCompare((b.fm.title as string) ?? ''),
    );
  }

  const sections: string[] = [];

  sections.push('## Overview & Hub Pages\n');
  sections.push(row(['Title', 'Status', 'Updated']));
  sections.push(row(['---', '---', '---']));
  if (byType.hub.length === 0) {
    sections.push(row(['_(none yet)_', '', '']));
  } else {
    for (const { rel, fm } of byType.hub) {
      sections.push(
        row([
          `[${fm.title ?? rel}](${rel})`,
          (fm.status as string) ?? '',
          (fm.last_updated as string) ?? '',
        ]),
      );
    }
  }

  sections.push('\n## Entities\n');
  sections.push(row(['Title', 'Scope tag', 'Status', 'Updated']));
  sections.push(row(['---', '---', '---', '---']));
  if (byType.entity.length === 0) {
    sections.push(row(['_(none yet)_', '', '', '']));
  } else {
    for (const { rel, fm } of byType.entity) {
      const tags = Array.isArray(fm.tags) ? fm.tags : [];
      const scopeTag = tags[0] ?? '';
      sections.push(
        row([
          `[${fm.title ?? rel}](${rel})`,
          scopeTag,
          (fm.status as string) ?? '',
          (fm.last_updated as string) ?? '',
        ]),
      );
    }
  }

  sections.push('\n## Concepts\n');
  sections.push(row(['Title', 'Tags', 'Status', 'Updated']));
  sections.push(row(['---', '---', '---', '---']));
  if (byType.concept.length === 0) {
    sections.push(row(['_(none yet)_', '', '', '']));
  } else {
    for (const { rel, fm } of byType.concept) {
      const tags = Array.isArray(fm.tags) ? fm.tags.join(', ') : ((fm.tags as string) ?? '');
      sections.push(
        row([
          `[${fm.title ?? rel}](${rel})`,
          tags,
          (fm.status as string) ?? '',
          (fm.last_updated as string) ?? '',
        ]),
      );
    }
  }

  sections.push('\n## Sources\n');
  sections.push(row(['Title', 'Status', 'Ingested']));
  sections.push(row(['---', '---', '---']));
  if (byType.source.length === 0) {
    sections.push(row(['_(none yet)_', '', '']));
  } else {
    for (const { rel, fm } of byType.source) {
      sections.push(
        row([
          `[${fm.title ?? rel}](${rel})`,
          (fm.status as string) ?? '',
          (fm.last_updated as string) ?? '',
        ]),
      );
    }
  }

  const output = [
    '# Wiki Index',
    '',
    '> Tables below are auto-generated by `llm-wiki-manager build`. Edit prose above this rule only.',
    '',
    '---',
    '',
    ...sections,
    '',
  ].join('\n');

  return { output, pages };
}

async function buildFinalIndexOutput(
  ctx: WikiContext,
): Promise<{ output: string; pages: PageEntry[] }> {
  const { wikiDir, repoRoot } = ctx;
  const { output: rawOutput, pages } = buildIndexOutput(wikiDir);
  const indexPath = join(wikiDir, 'index.md');
  const existing = existsSync(indexPath) ? readFileSync(indexPath, 'utf8') : '';
  const output = existing ? preserveIndexPreamble(rawOutput, existing) : rawOutput;
  const formatted = await formatIndexMarkdown(output, indexPath, repoRoot);
  return { output: formatted, pages };
}

export async function isIndexStale(wikiDir: string, repoRoot: string): Promise<boolean> {
  const ctx = resolveWikiContext({ repoRoot, wikiDir, cwd: repoRoot });
  const { output } = await buildFinalIndexOutput(ctx);
  const indexPath = join(wikiDir, 'index.md');
  const existing = existsSync(indexPath) ? readFileSync(indexPath, 'utf8') : '';
  return normalizeEol(existing) !== normalizeEol(output);
}

export async function runBuild(ctx: WikiContext): Promise<number> {
  const { wikiDir } = ctx;
  const { output, pages } = await buildFinalIndexOutput(ctx);
  const indexPath = join(wikiDir, 'index.md');
  writeFileSync(indexPath, output, 'utf8');
  console.log(`✓ index.md written (${pages.length} page(s))`);
  return 0;
}

export async function runCheck(ctx: WikiContext): Promise<number> {
  const { wikiDir } = ctx;
  const { output, pages } = await buildFinalIndexOutput(ctx);
  const indexPath = join(wikiDir, 'index.md');
  const existing = existsSync(indexPath) ? readFileSync(indexPath, 'utf8') : '';
  if (normalizeEol(existing) !== normalizeEol(output)) {
    console.error('✗ index.md is stale — run npm run wiki:build');
    return 1;
  }
  console.log(`✓ index.md is up to date (${pages.length} page(s))`);
  return 0;
}
