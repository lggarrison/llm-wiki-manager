...
Wiki subcommands accept `--wiki-dir` and `--repo-root`. Generated Markdown links from `build` and `sync` escape bracket characters in page titles before writing files. See [Wiki Management Scripts](../concepts/wiki-scripts.md) for flags and npm script aliases.
