...
Wiki subcommands accept `--wiki-dir` and `--repo-root`; `sync` treats `--dry` and
`--dry-run` as preview-only modes. See
[Wiki Management Scripts](../concepts/wiki-scripts.md) for flags and npm script
aliases.
