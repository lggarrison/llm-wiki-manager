...
`fs.test.ts` covers template copy, interpolation, install config, scaffold helpers, `.entity-scopes` preservation during upgrade refresh, managed `AGENTS.md` marker behavior (including fenced copied-marker examples), devDependency merging, and local binary detection.
