...
| ------------------------- | -------------------------------------------------------------------------------------------- |
| Init scaffold             | Creates wiki, `AGENTS.md`, install config, and `wiki:*` npm scripts                          |
...
| Legacy status migration   | Upgrade calls internal `runMigrate` to rewrite deprecated frontmatter (e.g. `draft` → `wip`) |
...
| UTF-8 BOM package.json    | Init succeeds; `doctor` reports no problems                                                  |
...
Packs the package with `npm pack`, installs it into a temp consumer project, and verifies `lint`, `build`, `check`, `sync`, and `doctor` work from the published tarball layout.
