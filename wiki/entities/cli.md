...
Wiki subcommands accept `--wiki-dir` and `--repo-root`. `sync` only writes generated links into real markdown sections outside fenced examples, and `log` accepts single-line titles so one command cannot forge additional entries. See [Wiki Management Scripts](../concepts/wiki-scripts.md) for flags and npm script aliases.
