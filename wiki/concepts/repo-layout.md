...
| `src/commands/upgrade.ts` | Upgrade command — preflight package version, refresh templates, migrate pages, sync scripts                  |
| `src/wiki/`               | Wiki lint, build, check, sync, log, help, doctor, setup-husky, migrate-pages implementations                 |
| `src/utils/fs.ts`         | Template copy, interpolation, package.json merge, AGENTS.md amend/managed replacement, install/config status |
