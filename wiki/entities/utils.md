...
type: overview
title: Utils
last_updated: 2026-08-14T11:14:00Z
tags: [utils]
...
| Install config | `readInstallConfig`, `writeInstallConfig`, `isExistingInstall`, `inferInstallConfig`; persisted configs are shape-validated before callers use scaffold fields                                                              |
...
`fs.test.ts` covers template copy, interpolation, install config shape validation, scaffold helpers, managed `AGENTS.md` marker behavior (including fenced copied-marker examples), devDependency merging, and local binary detection.
